// (C) 2011 Prevoow UG & Co. KG, Edward Ackroyd, Paderborn, Germany 
		...
		for (var i = 0; i < maxAds; i++) {
			...
			if (i == 0) {
				...
				if (responseInMs > RENDER_TOP_MAX_MS) {       
					...
					if (adPos == 0) {
						var taw = doc.getElementById("taw");
						if (taw) {
...
adaptGoogleHoverInit:function(doc) {
	try {
		var center_col = doc.getElementById("center_col");
		var w = doc.defaultView.getComputedStyle(center_col, null).width;
		if (w && w.indexOf("5")==0) {
			com.googlepreview.package.GP_addStyle("#cnt #center_col {width:635px;}", doc);
			//Fixed: "Translate this page" spoiling layout
			com.googlepreview.package.GP_addStyle("div.vsc>span {display:block;}", doc);
			//Fixed: Not enough space for Google's own hover over previews
			com.googlepreview.package.GP_addStyle("#nycp {margin-left:130px;}", doc);
			//Fixed: Results overlapping Google maps and ads
			com.googlepreview.package.GP_addStyle("#rhs_block {margin-left:70px;}", doc);
		}
	} catch (e) {;}
}
}
...
if (!u.match(/https?:\/\/(www|ipv6|encrypted)(|[0-9])\.(|l\.)google\..*\/.*/i)) {
	...
	com.googlepreview.package.GP_addStyle(".s {max-width:47em;}", document);
	com.googlepreview.package.GP_addStyle(".e {clear:left;}", document);
	com.googlepreview.package.GP_addStyle(".g {clear:left;}", document);
	com.googlepreview.package.GP_addStyle(".r {clear:left;}", document);
	com.googlepreview.package.GP_addStyle(".ts {clear:left;}", document);
	com.googlepreview.package.GP_addStyle(".tl {display:inline;}", document);
	
	//Google experimental "hover to init Google's own result previews"
	com.googlepreview.package.adaptGoogleHoverInit(document);
	
	var realQ = com.googlepreview.package.getGoogleQ(document).value;
	chrome.extension.sendRequest({command: "spads", qgi: realQ, location: document.location.href}, function(response) {
		if (response.xml && response.xml.length > 10) {
			com.googlepreview.package.adFeedRender(response.xml, response.rms, realQ);
		}
	});
